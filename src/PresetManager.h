...
{
    std::string name;
    float roomSize     = kParamRanges[PARAM_ROOM_SIZE].def;
    float stereo       = kParamRanges[PARAM_STEREO].def;
    float damping      = kParamRanges[PARAM_DAMPING].def;
    float hfColour     = kParamRanges[PARAM_HF_COLOUR].def;
    float earlyMix     = kParamRanges[PARAM_EARLY_MIX].def;
    float wetDry       = kParamRanges[PARAM_WET_DRY].def;
    float outputVolume = kParamRanges[PARAM_OUTPUT_VOLUME].def;
    float preDelay     = kParamRanges[PARAM_PREDELAY].def;
