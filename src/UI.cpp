...
//
// NOTICE: Do not confuse "scale mark" with "DPI scale"!
...
void ClassicReverbUI::onImGuiDisplay()
{
    ...
    if (ImGui::Begin("Main Window", nullptr, window_flags))
    {
                              ...
                              ImGuiWindowFlags_NoScrollWithMouse))
        {
            ...
            if (_BeginSection("SHAPING", SCALE((90.0f - 6.0f) * 2)))
            {
                ImGui::Dummy(ImVec2(SCALE(8), 0));
                ImGui::SameLine();

                _addKnob(PARAM_STEREO, "STEREO WIDTH",
                         kStereoMarks, IM_ARRAYSIZE(kStereoMarks));

                ImGui::SameLine(0, SCALE(20));
